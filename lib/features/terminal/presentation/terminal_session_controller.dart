...
class TerminalSessionController extends ChangeNotifier {
  ...
  void _configureTerminal() {
    ...
    terminal.onResize = (columns, rows, pixelWidth, pixelHeight) {
      ...
      // The view fires a burst of resizes while the keyboard animates and the
      // layout settles. Forwarding every one storms the server with SIGWINCH
      // and its repaints chase a moving target, leaving stale rows. Coalesce to
      // the final size once it stops changing.
